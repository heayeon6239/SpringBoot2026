...
//	MemberDAO memberdao;
	...
	public int signupConfirm(MemberDTO mdto) { // 자동 생성
		...
		if(isMember == false) {
			...
			int result = memberMapper.insertMember(mdto); // 수정된 값 들어감
...
//	}
	...
	public boolean modifyMember(MemberDTO mdto) {
		...
		if(dbPass != null && dbPass.equals(mdto.getPw())) {
			...
			return memberMapper.updateMember(mdto) == 1; // 1 이면 업데이트됨, 1 == 1 -> true !
		}else {
	...
	public boolean oneDelete(String id) {
		...
		return memberMapper.deleteMember(id) == 1; // 값이 일치하면 true
	}
	...
	public MemberDTO logincConfirm(MemberDTO mdto) {
		...
		MemberDTO result = memberMapper.oneSelectMember(mdto.getId());
